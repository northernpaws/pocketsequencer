...
impl<'a, DELAY: embedded_hal_async::delay::DelayNs> Audio<'a, DELAY> {
    pub async fn new(
        ...
        /*// Configure the codec registers with our required settings.
        codec
            .modify_audiointerface(|reg| {
                reg.with_wlen(0b11) // 32-bit word length
                    .with_aifmt(0b10) // set to i2s mode
            })
            .await?;

        // For testing, bypass DAC/ADC and route AUX input to output.

        // Enable and mix the L aux input into the left output mixer.
        codec
            .modify_leftmixer(|reg| {
                reg.with_lauxmxgain(0b101) // 0db
                    .with_lauxlmx(true) // enable left aux mix
            })
            .await?;

        // Enable and mix the R aux input into the right output mixer.
        codec
            .modify_rightmixer(|reg| {
                reg.with_rauxmxgain(0b101) // 0db
                    .with_rauxrmx(true) // enable right aux mix
            })
            .await?;
        ...
        codec
            .modify_aux1mixer(|reg| reg.with_rmixaux1(true))
            .await?;
        ...
        codec
            .modify_powermanagement1(|reg| {
                reg.with_aux1mxen(true) // Enable the aux1 mixer
                    .with_aux2mxen(true) // Enable the aux2 mixer
                    .with_pllen(true) // enable internal PLL; TODO: should be disabled once we have SAI master block established.
            })
            .await?;
        let after = codec.read_powermanagement1().await?;
        info!("before: {} after: {}", before.0, after.0);
        codec
            .modify_powermanagement2(|reg| {
                reg.with_rbsten(true) // Enable the right boost/mix ADC input
                    .with_lbsten(true) // Enable the left boost/mix ADC inpu
                    .with_rpgaen(true) // enable right channel PGA
                    .with_lpgaen(true) // enable left channel PGA
                    .with_radcen(true) // enable right channel ADc
                    .with_ladcen(true) // enable left channel ADc
            })
            .await?;
        codec
            .modify_powermanagement3(|reg| {
                reg.with_auxout1en(true) // enable the aux1 output
                    .with_auxout2en(true) // enable the aux2 output
                    .with_rmixen(true) // enable right output mixer
                    .with_lmixen(true) // enable left output mixer
                    .with_rdacen(true) // enable right DAC
                    .with_ldacen(true) // enable left DAC
            })
            .await?;
        ...
        codec
            .modify_inputcontrol(|reg| {
                // The internal microphone is connected to the left line-in PGA.
                reg.with_llinlpga(true) // Enable the L line-in PGA input
            })
            .await?;

        // Enable mixing the L and R aux input into the main ADC mixer at 0db.
        codec
            .modify_leftadcboost(|reg| reg.with_lauxbstegain(0b101))
            .await?;
        codec
            .modify_rightadcboost(|reg| reg.with_rauxbstgain(0b101))
            .await?;

        // Enable and mix the L aux input into the left MAIN output mixer.
        codec
            .modify_leftmixer(|reg| {
                reg //.with_lbypmxgain(0b101) // 0db
                    //.with_lbyplmx(true) // enable left ADC input bypass
                    .with_lauxmxgain(0b101)
                    .with_lauxlmx(true) // enable left AUX mix into left MAIN mixer
            })
            .await?;

        // Enable and mix the R aux input into the right MAIN  output mixer.
        codec
            .modify_rightmixer(|reg| {
                reg //.with_rbypmxgain(0b101) // 0db
                    //.with_rbyprmx(true) // enable right ADC input bypass
                    .with_rauxmxgain(0b101)
                    .with_rauxrmx(true) // enable right AUX mix into right MAIN mixer
            })
            .await?;
        ...
        // AUX1 can only connect to LMIX or RMIX.
        codec
            .modify_aux1mixer(|reg| reg.with_rmixaux1(true).with_rdacaux1(true))
            .await?;
