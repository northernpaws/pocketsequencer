...
pub async fn start_usb (spawner: Spawner, r: UsbResources) {
    ...
    let usb_driver: Driver<'static, embassy_stm32::peripherals::USB_OTG_HS> = get_usb_hs_driver(r, ep_out_buffer);
...
async fn midi_echo<'d, T: Instance + 'd>(class: &mut MidiClass<'d, Driver<'d, T>>) -> Result<(), Disconnected> {
    ...
    loop {
        let n = class.read_packet(&mut buf).await?;
        info!("received MIDI packet {=[u8]:b} {=[u8]:X} {}", &buf[..n], &buf[..n], &buf[..n]);
        ...
        if let Ok(event) = LiveEvent::parse(&buf[1..n]) {
            ...
            match event {
                LiveEvent::Midi { channel, message } => match message {
                    ...
                    MidiMessage::NoteOff { key, vel } => info!("MIDI: note off {} on channel {}", key.as_int(), channel.as_int()),
                    MidiMessage::Aftertouch { key, vel } => info!("MIDI: aftertouch {} on channel {}", key.as_int(), channel.as_int()),
                    MidiMessage::Controller { controller, value } => info!("MIDI: controller {}={} on channel {}", controller.as_int(), value.as_int(), channel.as_int()),
                    MidiMessage::ProgramChange { program } => info!("MIDI: program {} on channel {}", program.as_int(), channel.as_int()),
                    MidiMessage::ChannelAftertouch { vel } => info!("MIDI: aftertouch on channel {}", channel.as_int()),
                    MidiMessage::PitchBend { bend } => info!("MIDI: pitch bend {} on channel {}", bend.0.as_int(), channel.as_int()),
                },
                LiveEvent::Common(system_common) => match system_common {
                    midly::live::SystemCommon::SysEx(_) => info!("MIDI: SYS: SysEx"),
                    midly::live::SystemCommon::MidiTimeCodeQuarterFrame(_, u4) => info!("MIDI: SYS: quarter frame"),
                    midly::live::SystemCommon::SongPosition(u14) => info!("MIDI: SYS: song position: {}", u14.as_int()),
                    midly::live::SystemCommon::SongSelect(u7) => info!("MIDI: SYS: song select: {}", u7.as_int()),
                ...
                LiveEvent::Realtime(system_realtime) => match system_realtime {
                    midly::live::SystemRealtime::TimingClock => info!("MIDI: REALTIME: timing clock"),
                    ...
                    midly::live::SystemRealtime::ActiveSensing => info!("MIDI: REALTIME: active sensing"),
                    midly::live::SystemRealtime::Reset => info!("MIDI: REALTIME: reset"),
                    midly::live::SystemRealtime::Undefined(_) => info!("MIDI: REALTIME: undefined message"),
                },
            }
        } else {
